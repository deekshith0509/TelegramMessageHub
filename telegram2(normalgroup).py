"""
this is the major script that does the fowarding the files dynamically from the previously formatted objects which are JSONs.By default telegram (telethon) does not achieve the access to the oldest message instead it access from recent to oldest. So, I saved all those messages into JSON and then i forward the files from last object(JSON file)that too in reverse order.to make it just as a clone or copy..this way it maintains the structure of how the messages are forwareded.
